...
class CinemaShowsAnnotationView : MKPinAnnotationView {
    ...
    override func setSelected(_ selected: Bool, animated: Bool) {
        ...
        let scale:CGFloat = selected ? 1.0:0.7
        ...
        UIView.animate(withDuration: animated ? 0.15:0) {
            self.transform = CGAffineTransform(scaleX: scale, y: scale).concatenating(CGAffineTransform(translationX: translationX, y: translationY))//.translatedBy(x: translationX, y: translationY)
        }
