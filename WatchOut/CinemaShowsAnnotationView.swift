...
//        let translationX:CGFloat = selected ? 0:-frame.size.width*(1-scale)/2
        ...
        UIView.animate(withDuration: animated ? 0.15:0) {
            self.transform = CGAffineTransform(scaleX: scale, y: scale).concatenating(CGAffineTransform(translationX: 0, y: translationY))//.translatedBy(x: translationX, y: translationY)
        }
